...
public class SentEmbedInternal implements SentEmbed {
    ...
    public Builder toBuilder() {
        Builder builder = Embed.BUILDER().setTitle(title).setDescription(description).setUrl(Objects.nonNull(url) ? url.toExternalForm() : null).setTimestamp(
                timestamp).setColor(color).setFooter(Objects.nonNull(footer) ? footer.toDraft() : null).setImage(
                Objects.nonNull(image) ? image.toDraft() : null).setThumbnail(Objects.nonNull(thumbnail) ? thumbnail.toDraft() : null).setAuthor(
    ...
    public CompletableFuture<SentEmbed> modifyFields(Consumer<Collection<EmbedDraft.EditableField>> fieldModifier) {
        List<EmbedDraft.EditableField> fields = getFields().stream().map(SentEmbed.Field::toDraft).map(EmbedDraftInternal.EditableField::new).collect(Collectors
                                                                                                                                                              .toList());
...
// Override Methods
        ...
        public SentEmbed.Updater modifyFields(Consumer<Collection<EmbedDraft.EditableField>> fieldModifier) {
            Collection<EmbedDraft.EditableField> fields = builder.getFields().stream().map(EmbedDraftInternal.EditableField::new).collect(Collectors.toList());
            fieldModifier.accept(fields);
            builder.removeAllFields();
            fields.forEach(builder::addField);
            return this;
