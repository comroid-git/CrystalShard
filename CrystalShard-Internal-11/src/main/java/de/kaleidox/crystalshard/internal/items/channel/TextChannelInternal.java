...
public abstract class TextChannelInternal extends ChannelInternal implements TextChannel {
    ...
    public CompletableFuture<Message> sendMessage(EmbedDraft embedDraft) {
        if (checkPermissions()) return CompletableFuture.failedFuture(new DiscordPermissionException("Sending Message to Text Channel [" + id + "])",
                                                                                                     Permission.SEND_MESSAGES));
        return new WebRequest<Message>(discord).method(Method.POST).endpoint(Endpoint.Location.MESSAGE.toEndpoint(this)).node(objectNode("content",
                                                                                                                                         "",
                                                                                                                                         "embed",
    ...
    public CompletableFuture<Void> typing() {
        return new WebRequest<Void>(discord).method(Method.POST).endpoint(Endpoint.Location.CHANNEL_TYPING.toEndpoint(this)).execute(node -> null);
    }
