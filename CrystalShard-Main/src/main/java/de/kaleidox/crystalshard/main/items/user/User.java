...
// Static membe
    static CompletableFuture<User> of(UserContainer in, long id) {
        ...
        if (in instanceof Server) {
            ...
            userFuture = srv.getUserById(id).map(CompletableFuture::completedFuture).orElseGet(() -> new WebRequest<User>(discord).method(GET)
            ...
            userFuture = discord.getUserById(id).map(CompletableFuture::completedFuture).orElseGet(() -> new WebRequest<User>(discord).method(GET)
                    .endpoint(Endpoint.of(Endpoint.Location.USER, id))
                    .execute(node -> {
                        if (node.isObject()) {
                            return new UserInternal(discord, node);
    ...
    /**
     * Gets a user from only an ID. This method requires to be run from a Bot-Own Thread, see {@link ThreadPool#isBotOwnThread()}. This method must only be
    ...
    static CompletableFuture<User> of(long id) {
        ...
        if (ThreadPool.isBotOwnThread()) {
            ...
            userFuture = new WebRequest<User>(discord).method(GET).endpoint(Endpoint.of(Endpoint.Location.USER, id)).execute(node -> {
                if (node.isObject()) {
                    return new UserInternal(discord, node);
                } else throw new NoSuchElementException("No User with ID " + id + " found!");
            });
