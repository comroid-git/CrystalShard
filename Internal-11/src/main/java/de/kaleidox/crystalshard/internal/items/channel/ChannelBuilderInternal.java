...
// Override Methods
        ...
        public CompletableFuture<Invite> build() {
            return CoreDelegate.webRequest(Invite.class, channel.getDiscord()).method(Method.POST)
                    .endpoint(Endpoint.Location.CHANNEL_INVITE.toEndpoint(channel))
                    .node("max_age", maxAge, "max_uses", maxUses, "temporary", temporary, "unique", unique)
...
// Override Methods
        ...
        public CompletableFuture<ChannelCategory> build() {
            ...
            if (name == null) throw new IllegalArgumentException("No channel name set!");
            return CoreDelegate.webRequest(ChannelCategory.class, discord).method(Method.POST)
...
// Override Methods
        ...
        public CompletableFuture<ServerTextChannel> build() {
            if (!server.hasPermission(discord, Permission.MANAGE_CHANNELS)) return CompletableFuture.failedFuture(new DiscordPermissionException(
                    ...
                    Permission.MANAGE_CHANNELS));
            if (name == null) throw new IllegalArgumentException("No channel name set!");
            return CoreDelegate.webRequest(ServerTextChannel.class, discord).method(Method.POST)
                    .endpoint(Endpoint.Location.GUILD_CHANNEL.toEndpoint(server))
                    .node("type",
                          type,
                          "name",
    ...
    public static class ServerVoiceChannelBuilder extends ServerChannelBuilder<ServerVoiceChannel.Builder, ServerVoiceChannel>
        ...
        public CompletableFuture<ServerVoiceChannel> build() {
            if (!server.hasPermission(discord, Permission.MANAGE_CHANNELS)) return CompletableFuture.failedFuture(new DiscordPermissionException(
                    ...
                    Permission.MANAGE_CHANNELS));
            if (name == null) throw new IllegalArgumentException("No channel name set!");
            return CoreDelegate.webRequest(ServerVoiceChannel.class, discord).method(Method.POST)
                    .endpoint(Endpoint.Location.GUILD_CHANNEL.toEndpoint(server))
                    .node("type", type,
                          "name", name,
