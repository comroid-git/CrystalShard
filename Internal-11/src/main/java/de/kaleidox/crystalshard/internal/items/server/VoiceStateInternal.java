...
public class VoiceStateInternal implements VoiceState {
    ...
    private VoiceStateInternal(Discord discord, JsonNode data) {
        ...
        long serverId = data.get("guild_id").asLong(-1);
        this.server = serverId == -1 ? null : discord.getServerCache().getOrRequest(serverId, serverId);
        long userId = data.get("user_id").asLong();
        user = discord.getUserCache().getOrRequest(userId, userId);
        this.sessionId = data.get("session_id").asText();
        updateData(data);
        
        instances.entrySet().stream().filter(entry -> entry.getValue().getUser().equals(user)).forEach(entry -> instances.remove(entry.getKey(),
                                                                                                                                 entry.getValue()));
    ...
    public Set<EditTrait<VoiceState>> updateData(JsonNode data) {
        ...
        long channelId = data.path("channel_id").asLong(channel.getId());
        VoiceChannel newChannel = discord.getChannelCache().getOrRequest(channelId, channelId).toVoiceChannel().orElseThrow(AssertionError::new);
