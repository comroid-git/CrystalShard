...
// Override Methods
    ...
    public ServerMember.Updater addRole(Role role) throws DiscordPermissionException {
        if (!server.hasPermission(discord, Permission.MANAGE_ROLES)) throw new DiscordPermissionException("Cannot modify roles!", Permission.MANAGE_ROLES);
        if (role.equals(server.getEveryoneRole())) throw new IllegalArgumentException("Cannot add @everyone Role to a user!");
        this.roles.add(role);
        return this;
    }
    ...
    public ServerMember.Updater removeRole(Role role) throws DiscordPermissionException {
        if (!server.hasPermission(discord, Permission.MANAGE_ROLES)) throw new DiscordPermissionException("Cannot modify roles!", Permission.MANAGE_ROLES);
        if (role.equals(server.getEveryoneRole())) throw new IllegalArgumentException("Cannot remove @everyone Role from a user!");
    ...
    public ServerMember.Updater setMuted(boolean muted) throws DiscordPermissionException {
        if (!server.hasPermission(discord, Permission.MUTE_MEMBERS)) throw new DiscordPermissionException("Cannot mute other users!", Permission.MUTE_MEMBERS);
        this.muted = muted;
        return this;
    }
    ...
    public ServerMember.Updater setDeafened(boolean deafened) throws DiscordPermissionException {
        if (!server.hasPermission(discord, Permission.DEAFEN_MEMBERS)) throw new DiscordPermissionException("Cannot deafen other nickname!",
    ...
    public ServerMember.Updater moveTo(ServerVoiceChannel channel) throws DiscordPermissionException {
        if (!server.hasPermission(discord, Permission.MOVE_MEMBERS)) throw new DiscordPermissionException("Cannot move other users!", Permission.MOVE_MEMBERS);
    ...
    public CompletableFuture<Void> update() {
        return CoreDelegate.webRequest(discord).method(Method.PATCH)
