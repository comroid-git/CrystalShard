...
public class CustomEmojiInternal implements CustomEmoji {
    ...
    public CompletableFuture<Void> requestAllData() {
        return CoreDelegate.webRequest(discord).method(Method.GET).endpoint(Endpoint.Location.CUSTOM_EMOJI_SPECIFIC.toEndpoint(serverId, id)).execute().thenAccept(
                data -> {
                    data.path("role").forEach(node -> whitelistedRoles.add(discord.getRoleCache().getOrCreate(discord, server, node)));
                    this.creator = data.has("user") ? discord.getUserCache().getOrCreate(discord, data.path("user")) : null;
    ...
    public CompletableFuture<User> requestCreator() {
        ...
        return CoreDelegate.webRequest(User.class, discord).method(Method.GET)
    ...
    public CompletableFuture<Boolean> requestIsAnimated() {
        ...
        return CoreDelegate.webRequest(Boolean.class, discord).method(Method.GET).endpoint(Endpoint.Location.CUSTOM_EMOJI_SPECIFIC.toEndpoint(serverId, id)).execute(node -> {
