...
public class ChannelStructureInternal extends ArrayList<ServerChannel> implements ChannelStructure {
    ...
    public List<ServerTextChannel> getAllTextChannels() {
        return stream().filter(channel -> channel instanceof ServerTextChannel).map(ServerTextChannel.class::cast).collect(Collectors.toList());
    }
    
    @Override
    public List<ServerVoiceChannel> getAllVoiceChannels() {
        return stream().filter(channel -> channel instanceof ServerVoiceChannel).map(ServerVoiceChannel.class::cast).collect(Collectors.toList());
    }
    
    @Override
    public List<ChannelCategory> getAllCategories() {
        return stream().filter(channel -> channel instanceof ChannelCategory).map(ChannelCategory.class::cast).collect(Collectors.toList());
    }
}
