...
// Fields
    ...
    /**
     * A static implementation of the {@link DefaultEmbed#get()} method. This method will first check if the current thread is a {@link ThreadPool.Worker}
    ...
    public static EmbedDraft getStatic(Consumer<Embed.Builder> defaultEmbedModifier) {
        ...
        if (thread instanceof ThreadPool.Worker) {
            Embed.Builder builder = ((ThreadPool.Worker) thread).getDiscord().getUtilities().getDefaultEmbed().get().toBuilder();
    ...
    public static EmbedDraft getStatic() {
        ...
        if (thread instanceof ThreadPool.Worker) {
            return ((ThreadPool.Worker) thread).getDiscord().getUtilities().getDefaultEmbed().get();
        }
        throw new IllegalCallerException("The method DefaultEmbed#getStatic may only be called from a bot-own " +
                                         "thread, such as in listeners or scheduler tasks. You may not use it from " + "contexts like " +
                                         "CompletableFuture#thenAcceptAsync or such.");
    ...
    public static Embed.Builder getBuilderStatic() {
        ...
        if (thread instanceof ThreadPool.Worker) {
            return ((ThreadPool.Worker) thread).getDiscord().getUtilities().getDefaultEmbed().getBuilder();
        }
        throw new IllegalCallerException("The method DefaultEmbed#getStatic may only be called from a bot-own " +
                                         "thread, such as in listeners or scheduler tasks. You may not use it from " + "contexts like " +
                                         "CompletableFuture#thenAcceptAsync or such.");
    ...
    public static DefaultEmbed getInstance() {
        ...
        if (thread instanceof ThreadPool.Worker) {
            return ((ThreadPool.Worker) thread).getDiscord().getUtilities().getDefaultEmbed();
        }
        throw new IllegalCallerException("The method DefaultEmbed#getStatic may only be called from a bot-own " +
                                         "thread, such as in listeners or scheduler tasks. You may not use it from " + "contexts like " +
                                         "CompletableFuture#thenAcceptAsync or such.");
