...
public class NumberInput extends Base<Long> {
                    ...
                    event.getMessageAuthorUser().filter(participantTester::test).filter(usr -> !usr.isYourself()).ifPresent(author -> {
                        if (StringHelper.isNumeric(event.getMessageContent())) {
                            ...
                            if (val >= min && val <= max) {
                                ...
                                builder.setColor(Color.RED).setTitle("Error").addField("Error with the number!",
    ...
    public CompletableFuture<Integer> buildInteger() {
        return build().thenApply(lng -> {
            ...
            else throw new IndexOutOfBoundsException("The value is too " + (lng > Integer.MAX_VALUE ? "large" : "small") + "!");
        });
    }
}
