...
public class InformationMessage {
    ...
    public InformationMessage addField(String name, String title, String text, boolean inline) {
        fields.add(new InformationField(name, title, text, inline));
    ...
    public InformationMessage editField(String name, String newTitle, String newText, boolean newInline) {
        ...
        if (complex.isPresent()) {
            ...
            if (newTitle != null) field.setTitle(newTitle);
    ...
    public void refresh() {
        ...
        if (myMessage.get() != null) {
            myMessage.get()
                    .delete()
                    .thenRunAsync(() -> messageable.sendMessage(embed.build()).thenAcceptAsync(myMessage::set).exceptionally(Logger::handle))
                    .exceptionally(Logger::handle);
        } else {
            messageable.sendMessage(embed.build()).thenAcceptAsync(msg -> myMessage.set(msg)).exceptionally(Logger::handle);
        }
    }
