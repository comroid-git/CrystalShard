...
/**
 * This class represents a possible branch in a Discord Dialogue.
 * Each branch except the root branch results from a previous branch;
 * and is the product of a specified Option.
 * <p>
 * A Branch consists of a {@link ResponseElement} and a List of Options.
 * Those Options require a {@link Predicate} to test the result of the Branches
 * {@link ResponseElement}, and a sub-branch that will start if the {@link Predicate}
 * is turning out true.
 *
...
public class DialogueBranch<A> extends Dialogue {
     ...
     */
    public <B> DialogueBranch<A> addOption(
            Predicate<A> tester,
    ...
    protected void start(List<NamedItem> collectedItems) throws NullPointerException {
        questionElement
                .setParentBranch(this)
                .build()
                .thenAcceptAsync(response -> options
                        .stream()
    ...
    public class Option<B> implements Subclass {
        ...
        public Option(
                Predicate<A> tester,
                DialogueBranch<A> parentBranch,
                DialogueBranch<B> goToBranch) {
            this.tester = tester;
