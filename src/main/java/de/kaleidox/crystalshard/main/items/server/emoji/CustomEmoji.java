...
public interface CustomEmoji extends DiscordItem, Emoji {
     ...
     * {@link CompletableFuture#join()} upon calling this method. That way all future requests on this
     * CustomEmoji object like {@link #requestCreator} can be instantly used {@link CompletableFuture#get()},
     * or you can simply use the non-requesting methods from then on, like {@link #getCreator()}.
     * The returning future may be completed instantly, as the CustomEmoji might already be initialized with all data.
     *
     ...
     * Throws an exception if no data is available. Data can be requested available with {@link #requestAllData()}.
     *
    ...
    /**
     * Performs a request to acquire the creator of this emoji.
     * If there has been any request that includes this data made already, the returned future is completed instantly,
     * and you can use {@link #getCreator()} instead.
     *
    ...
    /**
     * Performs a request to acquire whether this emoji is animated.
     * If there has been any request that includes this data made already, the returned future is completed instantly,
     * and you can use {@link #isAnimated()} instead.
     * The method {@link #toDiscordPrintable()} always requires to know whether the emoji is animated, so it always
     * performs a blocking request on this method. This means, after calling {@link #toDiscordPrintable()}, this method
     * will always be completed once.
     *
    ...
    /**
     * Performs a request to acquire whether this emoji is managed.
     * If there has been any request that includes this data made already, the returned future is completed instantly,
     * and you can use {@link #isManaged()} instead.
     *
    ...
    /**
     * Performs a request to acquire whether this emoji requires to be wrapped in colons.
     * If there has been any request that includes this data made already, the returned future is completed instantly,
     * and you can use {@link #requireColons()} instead.
     * Usually, the result of this method is {@code TRUE} and describes a future feature.
     *
    ...
    /**
     * {@inheritDoc}
     * This method needs to know whether the emoji is animated, so it performs a blocking {@link #requestIsAnimated()}.
     */
