...
public final class GatewayMessageDeleteBulkPayload extends AbstractGatewayPayload {
    ...
    public static final VarBind<Long, DiscordBot, Message, Span<Message>> messages
            = Root.createBind("ids")
            .extractAs(ValueType.LONG)
            .andResolve((id, bot) -> bot.getSnowflake(Snowflake.Type.MESSAGE, id).get())
    ...
    public GatewayMessageDeleteBulkPayload(GatewayPayloadWrapper gpw) {
        ...
        getMessages().stream()
                ...
                .forEach(id -> getBot().getCache().remove(id, Snowflake.Type.MESSAGE));
    }
}
